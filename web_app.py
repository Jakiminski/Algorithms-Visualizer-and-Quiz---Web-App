...
# Inicialização das variáveis do Quiz (CORREÇÃO DO ERRO)
...
def render_quiz_visual(prob):
    ...
    if "array" in passo:
        ...
        for i, val in enumerate(passo["array"]):
            ...
            # Lógica simplificada de coloração baseada no algoritmo
...
if st.session_state.page == FRAME_MENU:
    ...
    if st.session_state.quiz_finished:
        ...
        for i, q in enumerate(st.session_state.quiz_questions):
            ...
            with st.expander(f"Questão {i+1} - {icon}"):
                ...
                # Renderizar estado visual para revisão
        ...
        with col_start:
            if st.button(BUTTON_QUIZ_INICIAR_QUIZ_TEXT, use_container_width=True):
                if not selected:
                    ...
                    # Configura Engine e Gera Perguntas
    ...
    # --- TELA DE JOGO (PERGUNTAS) ---
        ...
        # Renderização Visual
        with st.container(border=True):
            render_quiz_visual(question_data)

        # Opções de Resposta
        choices = question_data["choices"]
        options_list = [f"{k}) {v}" for k, v in choices.items()]
        
        # Se já respondeu, mostrar o que foi marcado
        ja_respondeu = current_idx in st.session_state.quiz_answers
        
        # O widget radio não suporta 'index=None' de forma limpa em versões antigas, 
        # mas podemos usar session_state para persistir seleção temporária
        ...
        # Feedback imediato se já respondeu
        if ja_respondeu:
        ...
        with c1:
            if not ja_respondeu:
                if st.button(BUTTON_QUIZ_CONFIRMAR_RESPOSTA_TEXT, use_container_width=True):
                    # Extrair letra da seleção "A) Valor" -> "A"
        ...
        with c3:
            # Se for a última questão e já respondeu, botão vira "FINALIZAR"
